...
[지난 편](/dev/dereel-1a-github-actions-크롤링-봇)에서 Apple 리퍼비시 재고 감시를 만들었습니다. 이번엔 범위를 넓혀 **게임 가격 감시**를 추가했습니다.
...
async def _fetch_bundle(self, bundle_id, name, currency, cc):
    ...
    # 개별 게임 원가 합산
    items = json.loads(str(bundle_div.get("data-ds-bundle-data", "{}")))
               .get("m_rgItems", [])
...
가격 형식이 특이합니다. `"4990 KRW"` — 센트 단위 정수 + 통화 코드 문자열입니다. (`4990` → ₩49.90 아니라 ₩49.90이 아니라 실제로는 ₩49.90... GOG는 1/100 단위 표기를 씁니다.)
